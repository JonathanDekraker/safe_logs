...
export default function ExportScreen() {
  ...
  useEffect(() => {
    generateExportData();
  }, [selectedPeriod, temperatureLogs, checklists, coolingLogs, sanitationLogs, monitoringLogs, correctiveActionLogs]);
  ...
  const generateExportData = () => {
    ...
    let data = `SafeLogs Export - ${formatDate(startTimestamp)} to ${formatDate(endTimestamp)}\n\n`;
    ...
    if (filteredTempLogs.length === 0) {
      ...
      filteredTempLogs.forEach(log => {
        const equipment = useAppStore.getState().equipment.find(e => e.id === log.equipmentId);
        data += `${formatDate(log.timestamp)} - ${equipment?.name || 'Unknown'}: ${log.temperature}°F `;
    ...
    if (filteredChecklists.length === 0) {
      ...
      filteredChecklists.forEach(checklist => {
        ...
        data += `${formatDate(checklist.date)} - ${checklist.type.toUpperCase()} Checklist: `;
    ...
    if (filteredCoolingLogs.length === 0) {
      ...
      filteredCoolingLogs.forEach(log => {
        data += `${formatDate(log.startTime)} - ${log.foodItem}\n`;
    ...
    if (filteredSanitationLogs.length === 0) {
      ...
      filteredSanitationLogs.forEach(log => {
        ...
        data += `${formatDate(log.timestamp)} - ${task ? task.name : "Unknown Task"}\n`;
    ...
    if (filteredMonitoringLogs.length === 0) {
      ...
      filteredMonitoringLogs.forEach(log => {
        ...
        data += `${formatDate(log.timestamp)} - ${plan?.name || "Unknown Plan"}: ${ccp?.name || "Unknown CCP"}\n`;
    ...
    const filteredCorrectiveActions = correctiveActionLogs.filter(
      log => log.timestamp >= startTimestamp && log.timestamp <= endTimestamp
    );
    ...
    if (filteredCorrectiveActions.length === 0) {
      ...
      filteredCorrectiveActions.forEach(log => {
        const plan = plans.find(p => p.id === log.planId);
        const ccp = plan?.criticalControlPoints.find(c => c.id === log.ccpId);
        ...
        data += `  Taken by: ${log.takenBy}\n`;
        if (log.followUpRequired) {
          data += `  Follow-up required: Yes\n`;
          data += `  Follow-up completed: ${log.followUpCompleted ? "Yes" : "No"}\n`;
          if (log.followUpDescription) data += `  Follow-up description: ${log.followUpDescription}\n`;
        } else {
          data += `  Follow-up required: No\n`;
        }
        data += `  Verified: ${log.verified ? "Yes" : "No"}\n`;
        if (log.verifiedBy) data += `  Verified by: ${log.verifiedBy}\n`;
        data += "\n";
